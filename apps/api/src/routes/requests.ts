...
const transitionRequestStatusSchema = z.object({
  toStatus: z.enum(['SOURCING', 'COMPLETED', 'CANCELED']),
...
export async function registerRequestRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/requests/:id/status', async (req, reply) => {
    ...
    const parsedBody = transitionRequestStatusSchema.safeParse(req.body);
    ...
    if (
      (toStatus === 'SOURCING' && request.status !== RequestStatus.FEE_PAID) ||
      (toStatus === 'COMPLETED' && request.status !== RequestStatus.PROPOSAL_PUBLISHED) ||
      (toStatus === 'CANCELED' &&
        (request.status === RequestStatus.CANCELED || request.status === RequestStatus.COMPLETED))
    ) {
      return reply.status(409).send({ error: 'INVALID_STATUS_TRANSITION' });
    }

    const fallbackReasonByStatus = {
      SOURCING: 'Operator moved request into active sourcing',
      COMPLETED: 'Operator marked proposal outcome as completed',
      CANCELED: 'Operator canceled request'
    } as const;

    const updatedRequest = await app.prisma.$transaction(async (tx) => {
      const nextRequest = await tx.sourcingRequest.update({
        where: { id: request.id },
        data: {
          status: toStatus
        }
